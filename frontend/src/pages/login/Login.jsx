    ...
    <div className="flex flex-col items-center justify-center min-w-96 mx-auto">
      <div className="w-full p-6 rounded-lg shadow-md bg-gray-400 bg-clip-padding backdrop-filter backdrop-blur-lg bg-opacity-0">
        <h1 className="text-3xl font-semibold text-center" style={{ color: "var(--clr-bitterseet)" }}>
          Login <span style={{ color: "var(--clr-cerulean)" }}>ChatApp</span>
        </h1>
        <form onSubmit={handleSubmit}>
          ...
          <Link
            to="/signup"
            className="text-sm hover:underline mt-2 inline-block" style={{ color: "var(--clr-light-orange)" }}
          >
            {"Don't"} have an account?
          </Link>
          <div>
            <button className="btn btn-block btn-sm mt-2 text-white b" disabled={loading}>
              {loading ? (
